...
| e | Claude `/login` carries to a new VM | **Pending.** Needs an interactive `/login` by the owner. |
...
| j | `sbx version --json` doesn't start the daemon | **Pending.** The daemon was already running; testing this means stopping it. |

## Go/no-go

The required checks (a), (b), (f) and (g) pass. (j) and (e) are still pending. No design change is needed; (c) and (h) select the fallbacks the spec already describes.
